...
- `render_analysis_context_pack_overview()` 仅公开白名单字段，不应把完整 pack、`items` 或 `value` 公开。
- `sanitize_context_snapshot_for_api()` 仅剥离 `analysis_context_pack_overview` 与 `market_phase_summary` 顶层字段，`context_snapshot` 其他字段保持历史兼容；敏感键的深度清洗仍依赖 `AnalysisContextPack.to_safe_dict()` 与 `redact_sensitive_mapping()` 链路。
