...
`SCHEDULE_TIME` currently only affects local built-in scheduling via `python main.py --schedule` and does not change GitHub Actions trigger timing. This change is documentation-only and does not implement runtime trigger control for GitHub Actions; it should not be treated as the delivery completion for `Issue #1497`.
