...
P6 只完成文档收口与核验路径，不改运行时契约：

- `pack`：保持 `src/schemas/analysis_context_pack.py`、`src/services/analysis_context_builder.py`、`src/analysis_context_pack_prompt.py`、`src/analysis_context_pack_overview.py` 的现有结构。
- `overview`：继续沿用 `analysis_context_pack_overview` 白名单字段暴露规范，补充文档级降级与核验要求。
- `prompt`：继续使用 `format_analysis_context_pack_prompt_section()` 的低敏摘要，不新增 runtime prompt 开关。
- `历史/API`：继续返回 `report.details.analysis_context_pack_overview` 与 `details.context_snapshot` 剥离规则，不暴露完整 pack 或敏感 payload。
...
- 本次收口未新增配置项，`P6` 期间不需要修改 `.env.example`，也不需要变更配置注册/加载链路。
- `docs/full-guide.md` 与 `docs/full-guide_EN.md` 为用户可见的使用说明同步补齐。
- 英文说明已同步更新；未进行单独英文策略文档重写。

### 回滚方式

- 若需回滚 P6 文档说明以外的运行行为，按发布策略回滚到对应版本即可（回退 `AnalysisContextPack` 相关历史提交）。
- 若仅需临时关闭文档面提示，直接恢复相关任务文档与变更说明；
- 无需额外的运行时 feature flag。
