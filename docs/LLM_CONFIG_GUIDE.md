...
- 对于上游拦截类文案，本次修复会将 `Your request was blocked.`、`Request was blocked.`、`request was blocked by policy`、`moderation_blocked` 等（来源可见 [Issue #1223](https://github.com/ZhuLinsen/daily_stock_analysis/issues/1223)）归入 `request_blocked + provider_blocked`；包含 `local firewall` 字样的本地网络拦截会保留为网络诊断分支，行为不触发配置清理或迁移。
