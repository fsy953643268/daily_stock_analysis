...
- [文档] 本次仅补充调度文档，并未实现 GitHub Actions 下 `SCHEDULE_TIME` 的运行时触发控制；`SCHEDULE_TIME` 仍仅影响本地内置调度（`python main.py --schedule`）。GitHub Actions 的触发时刻仍由 `00-daily-analysis.yml` 固定 `cron` 决定（周一到周五 10:00 UTC / 北京18:00）；如需调整请修改 workflow cron。该条目仅用于澄清边界，`Issue #1497` 对应的 Actions 调度能力实现仍待补充，本次仅为 docs 说明，不应视为该 Issue 交付闭环或关闭依据。
